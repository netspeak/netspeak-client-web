...
export const NetspeakGraphBody = (props: NetspeakGraphBodyProps): JSX.Element => {
	...
	if (props.orderWordsFromTop) {
		const maxY = findPositions(35 + maxFontSize / 2);
	} else {
	...
	useEffect(() => {
		...
		links.forEach(linkData => {
			...
			if (source == undefined || target == undefined) {
				...
				const sourceColumnBox = (d3
					.select("#column" + linkData.sourceColumnIndex)
					.node() as SVGSVGElement).getBBox();
	...
	}, [props.columns, width, links, maxFontSize, MAX_ROWS, ORDER_TOP_TO_BOTTOM, drawLinkThroughPositions]);
		...
		<svg viewBox={"0 0 " + width + " " + height} id="graphSVG" key="graphSVG" preserveAspectRatio="xMidYMin meet">
			{
				// Draw
				props.columns.map((column, cIndex) => {
					const linksXPositions = [
						(width / props.columns.length) * (cIndex - 0.15),
						(width / props.columns.length) * (cIndex + 0.15),
					];
					//Draw columns
					return (
